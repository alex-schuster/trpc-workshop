...
const filterFlights = publicProcedure.input(filterFlightsInput).query(async ({ ctx, input }) => {
  const flights = await ctx.prisma.flight.findMany({
    where: {
      originId: input.originCode,
      destinationId: input.destinationCode,
      departureAt: input.departureAfter ? { gte: new Date(input.departureAfter) } : undefined,
      arrivalAt: input.arrivalBefore ? { lte: new Date(input.arrivalBefore) } : undefined,
...
const bookFlight = publicProcedure.input(bookFlightInput).query(async ({ ctx, input }) => {
